...
* C++另一种编程思想称为 ==泛型编程== ，主要利用的技术就是模板。其实就是JAVA里面的泛型！！
...
建立一个通用函数，其函数返回值**类型**和**形参类型**可以不具体制定，用一个**虚拟的类型**来代表。
...
typename  --- 表明其后面的符号是一种数据类型，这里也可以用class代替
...
* 显示指定类型推导，模板必须要提前**确定出T的数据类型**，才可以使用
...
* 函数模板调用时，如果利用**显示指定类型**的方式，可以发生隐式类型转换
...
1. 如果存在两个名字、参数都相同的的函数模板和普通函数，会优先调用**普通函数**
...
**因此C++为了解决这种问题**，提供**模板的具体化操作**，可以为这些**特定的类型**提供**具体化的模板**
