...
在workerManager.cpp中提供具体的功能实现，`exit(0);————任意地方执行，退出整个系统`
...
如果想在程序中维护这个不定长度的数组，可以**将数组创建到堆区，并利用Worker * *的指针维护**。   

**开辟到堆区的原因**是：我们要在多个函数里面使用这个数据，假如开在栈区，那个包含开辟数据代码的函数执行完了之后，这些数据就没得了



==`Worker**  pWorker= new Worker *[5]`==，用的是指针数组对吧，所以接收的为这个指针数组的指针。就相当于我们把Worker*整体当成一个数据类型（如int）就好理解。

那不开辟到堆区的话，写法就是：==`Worker* workerArr[5]`==，数组名字为workerArr的指针数组。
...
WorkerManager::~WorkerManager()
{
	...
	if (this->m_EmpArray != NULL) {
		...
		//这里为什么好需要清空呢？？？哦哦我们底层对象是new的，我们的指针数组也是new的
		delete[] this->m_EmpArray;
	}
}
...
在workerManager.h中添加**成员函数** ` int get_EmpNum();`
...
在workerManager.cpp构造函数中在后面继续追加代码：
	...
	if (select == 1)
	{
		...
		if (this->m_EmpArray != NULL)
		{
            for (int i = 0; i < this->m_EmpNum; i++)
			{
				if (this->m_EmpArray[i] != NULL)
				{
					delete this->m_EmpArray[i]; // 清空堆区的对象
				}
			}
            //这里为什么好需要清空呢？？？哦哦我们底层对象是new的，我们的指针数组也是new的
            delete[] this->m_EmpArray;
