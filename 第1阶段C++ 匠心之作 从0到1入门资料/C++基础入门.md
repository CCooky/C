...
**作用**：给一段指定的内存空间起名，方便操作这段内存，这个数据在内存空间是有地址的啊！！！，我们不使用这个地址，因为太麻烦了。

<img src="images/image-20220929170146877.png" alt="image-20220929170146877" style="zoom:80%;" />

**语法**：`数据类型 变量名 = 初始值;`

**信息：** 直接输出a——数据；输出&a——数据地址
...
**作用**：C++规定给**标识符（变量、常量）**命名时，有一套自己的规则
...
**字节byte：**计算机中数据的最小组成单位：8个二进制位为一组，来保存数据，叫做一个字节byte。其中，每个二进制位称为一个比特bit，即1byte = 8bit。
...
PS： 11两位最大为3 = 2^2-1，最小为 -4

**为什么int 为什么最小值是-32768呢**

语言中，int若是规定大小为两个字节，则其最小值就是-32768。
...
**作用：**利用sizeof关键字可以==统计数据类型/ 变量所占内存大小==，输出为字节数
...
- 可以直接用ASCII给字符型变量赋值！！最后就是那个字符，`char bb = 98;`
...
2. 对数组名进行size，可以获取整个数组数据个数；
2. (int)&arr[0]——数组中第一个元素地址
...
3. `数据类型  数组名[ 行数 ][ 列数 ] = { 数据1，数据2，数据3，数据4};` 会按照左上---右下放数据
4. ` 数据类型  数组名[  ][ 列数 ] = { 数据1，数据2，数据3，数据4};`自动识别行数大小；
...
> 总结：在定义二维数组时，如果初始化了数据，可以省略行数,他会自动知道你是几行。
...
* 对sizeof（arr[0]）——二维数组第一行占用的内存空间大小
...
​		我们c++的从上往下编译的，并且我们main函数都写在最后面，假如我们把main写在了第一个位置，就会报错说找不到max函数，此时函数的 声明就是提前告诉main，我们有这个函数
...
1. 创建后缀名为**.h的头文件**  ，包含要使用的头文件、namespace。
2. 创建后缀名为**.cpp的源文件** ，包含创建的头文件 `#include "swap.h"`
3. **头文件**===>>>**写函数的声明**
4. **源文件**===>>>**写函数的定义**
4. **main文件**===>>>正常的基础上，额外包含头文件`#include "swap.h"`，直接用写好的函数即可
...
* 内存编号是从0开始记录的，一般用十六进制数字表示，也可以用（int）转为十进制查看
* 可以利用指针变量保存地址编号


<img src="images/image-20221001181519028.png" alt="image-20221001181519028" style="zoom:67%;" />
...
* 指针变量可以通过" * "操作符，操作指针变量指向的内存空间，即得到内存里面存储的数据，这个过程称为==解引用==
* 可以直接通过解引用修改内存里面数据的值，就像是这个数据对应的变量名一样，可以直接操作这个内存。
...
> 总结：所有指针类型在32位操作系统下是4个字节，64位的系统为8个字节。
...
<img src="images/image-20221001184315712.png" alt="image-20221001184315712" style="zoom:80%;" />

> const int * p1 = &a; 
>
> 指针的指向就是指的 指针存放的地址值，指针就是一个变量撒，对应一个内存空间
>
> 指针指向的值不可以改——不能通过解引用去修改内存数据
>
> 技巧：const修饰的是*，那么 *的操作不能改，也就是 *P不能改

<img src="images/image-20221001185107947.png" alt="image-20221001185107947" style="zoom: 80%;" />

> int * const p2 = &a;
>
> 技巧：const修饰的是p，那么 p的操作不能改，也就是 P存放的地址不能改

<img src="images/image-20221001185216830.png" alt="image-20221001185216830" style="zoom:80%;" />
...
int main() {
	...
	int * p = arr;  //指向数组的指针，这里指针的数据类型和要指向的内存数据类型一致
...
<img src="images/image-20221001191943288.png" alt="image-20221001191943288" style="zoom:80%;" />
...
![image-20221019160031976](images/image-20221019160031976.png)
